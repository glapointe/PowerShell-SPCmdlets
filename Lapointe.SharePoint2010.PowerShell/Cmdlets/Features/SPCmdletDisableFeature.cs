...
{
    [Cmdlet("Disable", "SPFeature2", SupportsShouldProcess = false),
    SPCmdlet(RequireLocalFarmExist = true, RequireUserMachineAdmin = true, RequireUserFarmAdmin = true)]
    [CmdletGroup("Features")]
    [CmdletDescription("Deactivates a feature or features at a given scope.")]
    [RelatedCmdlets(typeof(SPCmdletEnableFeature),
        ExternalCmdlets = new[] {"Disable-SPFeature", "Enable-SPFeature", "Get-SPFeature"})]
    [Example(Code = "PS C:\\> Get-SPFeature MyFeature | Disable-SPFeature2 -Url \"http://server_name\" -IgnoreNonActive -DeactivateAtScope WebApplication",
        Remarks = "This example deactivates all occurences of MyFeature in the http://server_name web application.")]
    public class SPCmdletDisableFeature : SPSetCmdletBaseCustom<SPFeatureDefinition>
    {
        bool farmLevelFeature = false;
        SPSite m_Site;
        
        [Parameter(Mandatory = false,
            HelpMessage = "If provided, the cmdlet outputs the Feature definition object after disabling.")]
        ...
        [Parameter(Mandatory = true,
            ...
            Position = 0,
            HelpMessage = "Specifies the name of the Feature or GUID to uninstall.\r\n\r\nThe type must be the name of the Feature folder located in the 14\\Template\\Features folder or GUID, in the form 21d186e1-7036-4092-a825-0eb6709e9281.")]
        public SPFeatureDefinitionPipeBind Identity { get; set; }

        [Parameter(Mandatory = false,
            HelpMessage = "Specifies the URL of the Web application, site collection, or Web site for which the Feature is being deactivated.\r\n\r\nThe type must be a valid URL; for example, http://server_name.")]
        public string Url { get; set; }

        [Parameter(Mandatory = false,
            HelpMessage = "Forces the deactivation of a Feature. This causes any custom code associated with the Feature to rerun.")]
        ...
        [Parameter(Mandatory = false,
            HelpMessage = "Deactivate features at the specified scope only. Valid values are Farm, WebApplication, Site, Web, and Feature")]
        public ActivationScope? DeactivateAtScope { get; set; }
